...
- 当前已提供 `/api/setup/from-sync` 的 master-password bootstrap-only 后端边界，并已交付 recovery key 的一次展示、离线确认、错误输入保护和包装轮换；尚无独立本地 Vault 的新设备恢复 UI，真实删除 re-auth 和完整账号删除闭环也未交付，不能把账号登录误认为可以自动恢复 Vault。
