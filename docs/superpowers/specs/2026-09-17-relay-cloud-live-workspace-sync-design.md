...
每个安装实例有自己的本地工作区和本地 SSH 执行端；同一账号的 Host、Identity、Group、Snippet 等持久配置则是一份账号级数据，登录并完成设备信任后自动同步到各端，效果与同一 Web 服务的不同浏览器看到同一账号数据一致。登录同一账号后，首页显示“当前端”以及该账号下其他端的工作区，例如“Web”“Android 小明”“Windows 办公室”。点“Windows 办公室”进入它**当前正在运行的工作区**：若该端已连接两个终端，当前端显示这两个终端的实际画面，输入转发给 Windows 端已有的 SSH 连接，输出同步回所有观看端。远端 Host、SFTP、命令与工作区操作由远端执行端处理；当前端不复制远端 Shell，也不在自己设备上重新连接目标 SSH 主机。
...
云服务保存两类**端到端加密持久数据**：账号级配置快照（Host、Identity、Group、Snippet 与终端配置，所有受信设备自动收敛到同一版本）和设备级工作区状态快照（布局、标签、打开意图等，按设备分别保存并供其他端查看）。主题偏好是否跟随账号须由独立字段契约定义；现有 v1 快照不含主题，不能假定它已经同步。快照和实时会话是两条通道：快照不包含 live Shell、终端原始内容或 SFTP 文件；实时流不作为云端历史记录。远端执行端离线或 Vault 锁定时，不能伪装终端可操作：显示离线/锁定原因、最后同步时间，可查看已授权的最近工作区快照，所有远程操作禁用。客户端的“当前端”始终可独立工作。

“远端 A/B”在本设计中是设备或 Web 执行实例拥有的工作区，而非云端运行的 SSH 会话。首次打开其他端先验证账号、设备信任和解密能力；之后直接进入该端实时工作区。进入远端不触发 Host 等账号数据复制：这些配置本就通过账号级同步自动共享。设备级布局/打开意图不覆盖当前端；如需复制设备工作区布局，应使用单独的显式操作。
...
本设计引入 v2 云协议与独立部署；不把现有 SQLite 表替换成 MySQL 连接串，也不直接暴露旧的 `/api/sync/v1/envelope` 为多工作区 API。旧 Web v1 快照须拆成一份账号级配置和该 Web 实例的一份设备级工作区状态，保留可回滚副本。Web 当前 `ownerId='default'`/单 Vault 假设需要隔离成每账号独立本地数据域；在完成隔离前不得让多个账号共用同一 Web 执行实例的数据与 live Shell。
...
MySQL InnoDB 使用独立 schema。账号级数据有唯一 head，设备级工作区各有自己的 head；所有关联均以 `account_id` 限定，所有写入有幂等键和审计摘要。
...
| `account_data_keys` | `account_id, key_version, device_id` | 向受信设备封装的账号配置密钥；云端不持有解密 key |
| `account_data_heads` / `account_data_revisions` | `account_id`；`account_id, revision` | 全端共享的加密配置快照、CAS、作者设备和幂等键；同账号受信端均可写 |
...
MySQL 持久化账号、设备、加密快照与审计；live 输出只在在线内存中转发，禁止写 MySQL、普通日志或备份。发布账号级或设备级快照时，以 InnoDB 事务锁定对应 head、校验 `parentRevision`，再写新 revision/head；版本不匹配返回冲突，不做最后写入覆盖。账号级配置由多端共同编辑，必须处理两端离线编辑分叉；设备级工作区只有拥有者可发布，冲突主要来自其离线队列、重试或迁移分叉。事务与行锁行为按 [MySQL InnoDB 文档](https://dev.mysql.com/doc/refman/8.4/en/innodb-transaction-model.html) 验证。v1 先用快照 CAS 和人工冲突处理；对象级自动合并留后续。

云 API v2：`/v2/auth/*` 登录、刷新、登出；`/v2/devices` 列表/重命名/撤销；`/v2/account-data/head|revisions|snapshot` 获取账号级加密配置、`PUT /v2/account-data/snapshot` 带 `parentRevision` 和幂等键发布；`/v2/workspaces` 列表与权限；`/v2/workspaces/{id}/descriptor|head|revisions|snapshot` 获取设备级加密状态，`PUT /v2/workspaces/{id}/snapshot` 仅拥有者可发布；`/v2/relay/owner` 与 `/v2/relay/viewer` 为已认证 WSS。所有路由从会话解析 account/device，不接受客户端自报的任意账号 ID；IDOR、超额载荷和已撤销设备应返回稳定错误码。服务端列表只返回最小元数据，不暴露 Host 地址、命令、私钥或终端输出。

## 5. 密钥、登录与恢复

各端本地 Vault 独立，但账号级配置在解锁且完成同步后保持同一逻辑数据集。首次开通云同步时客户端生成账号同步根密钥 `K_account`，由其包装账号配置密钥 `K_data`；每个设备级工作区另有 `K_workspace`。两类快照分别由对应密钥 AEAD 加密，密钥仅向授权设备封装；AAD 绑定协议版本、accountId、数据域（`account-data` 或 workspaceId）、revision、parentRevision、keyVersion、writerDeviceId。云端存密文、包装密钥和最小路由元数据，不能解密快照。旧 v1 `K_sync`/Vault 包装不可无条件沿用为 v2 密钥；客户端在用户确认迁移时拆分并重新加密两类快照，保留 v1 回滚副本。

账号登录只证明身份；新设备仍需离线恢复密钥或已信任设备批准以取得 `K_account`，且本地 Vault 解锁后才能查看/应用敏感配置。Windows/Android 的设备私钥与会话凭据进 OS keychain/Android Keystore；Web 执行实例安全保存设备凭据，浏览器只持有 HttpOnly 会话。设备撤销立即停止新的快照读取/写入与 live relay，强制关闭现有订阅，随后轮换账号配置及受影响工作区的密钥并为剩余设备重封装。撤销不能使已被该设备下载并解密的旧数据失效；UI 应说明这个边界。丢失全部恢复材料时账号密码重置不恢复加密工作区。

实时 Console 流也视为敏感数据：所有参与端先完成设备互信，拥有者与每个观看设备分别协商临时加密会话密钥；加入、离开、撤销或重连须使旧会话密钥失效，拥有者分别封装发往各观看端的流。云服务只转发密文帧及必要路由元数据，不记录终端字节或输入。实现前用 Web 执行端、Electron 和 Android 真机验证同一加密帧格式、密钥轮换与断线重协商；无法通过时不得以明文中继冒充端到端加密。云端仍能看到在线状态、设备/工作区 ID、流量时间和大小，这些属于明示的元数据。
...
4. 多端输入帧含 `participantDeviceId/inputId/sessionId/payload`，仅拥有者能写 SSH 通道。拥有者校验授权、长度、控制字符策略与 Vault/session 状态，为每帧分配 `inputSequence`，去重后按序写入并确认；所有参与端显示操作人。粘贴按一帧处理并限制大小；断线后未确认输入不自动重放，避免命令重复执行。如果 SSH 已接受输入但确认帧丢失，观看端显示“执行结果未知”，而非“失败可重试”；用户自行检查终端后决定是否重发。云服务不修改或执行输入。
...
任一受信端本地修改 Host/Identity/Group/Snippet/终端配置后，先提交本地事务，再按 `parentRevision` 发布账号级加密配置快照；其他端拉取、解密并原子应用，在线时自动同步，离线时保留本地改动与加密待发布队列，恢复联网后处理冲突。设备拥有者修改自己的布局、标签和打开意图时另行发布设备级快照；观看端在远端工作区发起的操作由拥有者执行，工作区状态也由拥有者发布。账号数据自动共享，不需要“复制配置到当前端”；只在用户明确要求复制某端布局时预览并应用设备级状态。登录、同步未完成或 Vault 锁定时必须显示准确状态，不以旧数据冒充已同步。

如果两个设备离线修改账号级数据造成 head 分叉，停止自动覆盖，在客户端保留本地与云端两份加密快照并提供“保留本端、采用云端、加密导出两份”；未被 CAS 接受的分支不得假称已存入云 revision。Host Key、凭据、ProxyJump 和命令片段不能静默按时间合并。设备级快照只接受拥有者发布，其队列与云 head 分叉时采用同样的显式冲突处理。删除工作区使用软删除、保留期和设备撤销检查；恢复不自动重建已失效的 live session。Web 旧 v1 迁移须记录旧 head/hash 到新账号配置 revision 与 Web workspace revision 的映射、加密备份、幂等迁移标记和回滚窗口；不能把 SQLite 中的现有账号/同步表当成云 MySQL 的生产多用户数据。
...
- 同一账号在三端修改 Host、Identity、Group 或 Snippet 后，其余在线端自动得到同一账号配置；离线端恢复联网后可同步，冲突不得静默覆盖；各端的终端布局和真实 SSH session 仍独立。
...
- 设备撤销立即断开 live 订阅，拒绝后续快照请求；两设备修改账号级配置造成的 revision 冲突可导出两份并安全恢复。云服务不可用时当前端仍可工作，远端入口明确不可实时进入。
