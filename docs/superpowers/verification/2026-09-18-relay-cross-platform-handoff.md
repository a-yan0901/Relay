...
## 65. 2026-09-20 服务端 unit/integration 全量定向复验
...
- 当前工作区执行 Web 终端恢复、服务端 Vault bundle、Windows local runtime 和 Windows native file services 定向套件，命令为 `npm test -- --run tests/unit/web/app-terminal-lifecycle.dom.test.tsx tests/unit/server/vault-bundle.test.ts tests/unit/windows/local-runtime.test.ts tests/unit/windows/native-file-services.test.ts --no-file-parallelism --maxWorkers=1 --reporter=dot`。
- 结果为 `4` 个测试文件、`23` 个测试全部通过；该证据不替代 Android 真机 A-01～A-17、A-17 双向实机 bundle、Windows 原生系统对话框人工走查或正式签名。
...
- 隔离 `createWindowsLocalRuntime({ dataDir: ':memory:' })` 对错误密码和篡改 ciphertext 均返回 `VAULT_BUNDLE_INVALID`，拒绝后 hosts/groups/identities 保持 `0/0/0`；正确预览/应用为 `3/2/2`。
- `Web CDP Handoff Host` 的名称、地址、端口、用户名和 `password` 认证类型核对通过；Profile 为 `5` 个内置加 `1` 个自定义 `vector-terminal-profile`，无重复导入。该证据补齐 Android→Windows local runtime 当前回环，但不替代 Windows 打包 UI 导入、两台真机和正式签名。
...
- Android 原生 bridge 对错误密码和篡改 ciphertext 均返回 `VAULT_BUNDLE_INVALID`，拒绝后 Host 数量保持 `3`；正确预览/应用为 `1` Host，数量增至 `4`，并核对名称、地址、端口、用户名和 `password` 认证类型。
- 该条补齐模拟器 Windows local runtime→Android 实际回传；A-17 仍不标记通过，Windows 打包 UI 导入、两台真机、原生系统对话框人工走查和正式 Authenticode 仍未完成。

## 2026-09-20 Android SAF 取消结果 URI grant 防御修复

- 发现并修复：文件选择/保存 Activity 在非 `RESULT_OK` 但仍携带 content URI 时，旧逻辑会先丢弃 URI，无法执行失败态 grant 回收。现在统一解析 URI，按实际 READ/WRITE mode（缺失时使用 open/save fallback）回收；成功、失败、取消、executor 不可用以及 writer close/cancel 生命周期均有对应释放路径。
