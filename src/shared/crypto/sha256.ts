...
export class Sha256 {
  ...
  update(input: Uint8Array): this {
    ...
    if (this.bufferLength > 0) {
      const needed = Math.min(64 - this.bufferLength, input.byteLength);
      this.buffer.set(input.subarray(0, needed), this.bufferLength);
    ...
    if (offset < input.byteLength) {
      this.buffer.set(input.subarray(offset), this.bufferLength);
  ...
  digest(): Uint8Array {
    ...
    copy.state.forEach((value, index) => view.setUint32(index * 4, value));
    return result;
  }
