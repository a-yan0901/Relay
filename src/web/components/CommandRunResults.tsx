...
export const CommandRunResults = ({ run, hosts, onCancel, onOpenHost }: CommandRunResultsProps) => {
  ...
  const pageCount = Math.max(1, Math.ceil(filteredTargets.length / RESULT_PAGE_SIZE));
  const pagedTargets = useMemo(() => filteredTargets.slice(page * RESULT_PAGE_SIZE, (page + 1) * RESULT_PAGE_SIZE), [filteredTargets, page]);
  ...
  return (
    <section className="command-run-results" aria-label="批量执行结果">
      ...
      {filteredTargets.length === 0 ? <p className="target-picker-empty">没有匹配的执行结果</p> : <>
        {pageCount > 1 && <nav className="command-result-pagination" aria-label="结果分页"><span>第 {page + 1} / {pageCount} 页 · {filteredTargets.length} 条</span><button className="button button-ghost button-small" type="button" disabled={page === 0} onClick={() => setPage((current) => Math.max(0, current - 1))}>上一页</button><button className="button button-ghost button-small" type="button" disabled={page >= pageCount - 1} onClick={() => setPage((current) => Math.min(pageCount - 1, current + 1))}>下一页</button></nav>}
        <ul>{pagedTargets.map((target) => {
        ...
        return <li key={target.hostId} className={`command-target command-target-${target.status}`}><div className="command-target-heading"><label className="command-target-select"><input type="checkbox" aria-label={`选择对比 ${label}`} checked={selectedHostIds.has(target.hostId)} onChange={() => setSelectedHostIds((current) => toggleSetValue(current, target.hostId))} />对比</label><div><strong>{label}</strong><small>{host?.address ?? '未知主机'} · {targetStatusLabels[target.status]}{target.errorCode === 'SERVICE_RESTARTED' ? ' · 服务已重启' : ''}{target.exitCode === null ? '' : ` · exit ${target.exitCode}`}</small></div></div><div className="command-target-actions"><span>{target.outputBytes.toLocaleString()} bytes{target.truncated ? ' · 服务端已截断' : ''}</span><div className="command-target-buttons">{onOpenHost && host && <button className="button button-ghost button-small" type="button" aria-label={`打开主机 ${label}`} onClick={() => onOpenHost(target.hostId)}>打开主机</button>}<button className="button button-ghost button-small" type="button" aria-label={`${expanded ? '收起输出' : '查看输出'} ${label}`} aria-expanded={expanded} onClick={() => setExpandedHostIds((current) => toggleSetValue(current, target.hostId))}>{expanded ? '收起输出' : '查看输出'}</button></div></div>{expanded && <pre>{displayOutput || '（无输出）'}</pre>}</li>;
        })}</ul>
      </>}
    </section>
  );
