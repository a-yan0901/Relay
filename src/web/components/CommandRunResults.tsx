...
const targetStatusLabels: Record<CommandRun['targets'][number]['status'], string> = {
  queued: '排队中',
  running: '执行中',
  completed: '已完成',
  failed: '失败',
...
export const CommandRunResults = ({ run, hosts, onCancel }: CommandRunResultsProps) => {
  ...
  return (
    <section className="command-run-results" aria-label="批量执行结果">
      ...
      <ul>{run.targets.map((target) => { const host = names.get(target.hostId); return <li key={target.hostId} className={`command-target command-target-${target.status}`}><div><strong>{host?.name ?? target.hostId}</strong><small>{host?.address ?? '未知主机'} · {targetStatusLabels[target.status]}{target.errorCode === 'SERVICE_RESTARTED' ? ' · 服务已重启' : ''}{target.exitCode === null ? '' : ` · exit ${target.exitCode}`}</small></div><pre>{target.output}{target.truncated ? '\n… 输出已截断' : ''}</pre></li>; })}</ul>
    </section>
  );
};
