...
describe('TerminalPanel mobile selection', () => {
  ...
  it('updates xterm colors and font size when the application theme changes', () => {
    const { rerender } = render(<TerminalPanel terminalId="terminal-theme" host={host} active onClose={() => {}} preferences={{ theme: 'termius', fontSize: 13 }} />);
    const terminal = testState.terminalInstances[0] as unknown as { options: { theme: { background?: string }; fontSize: number } };
    expect(terminal.options.theme.background).toBe('#141728');
    expect(terminal.options.theme.foreground).toBe('#5cc97c');
    rerender(<TerminalPanel terminalId="terminal-theme" host={host} active onClose={() => {}} preferences={{ theme: 'tokyo-day', fontSize: 16 }} />);
    expect(terminal.options.theme.background).toBe('#e1e2e7');
    expect(terminal.options.theme.foreground).toBe('#3760bf');
